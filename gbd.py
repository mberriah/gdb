...
def get_book_data(url):
    ...
    driver.refresh()
    sleep(3)
    title = driver.find_element(by=By.CLASS_NAME, value="gb-volume-title").text
    author = driver.find_element(by=By.CLASS_NAME, value="addmd").text
...
def capture_requests(url):
    ...
    while checkpoint != driver.find_element(by=By.CLASS_NAME, value="pageImageDisplay"):
        checkpoint = driver.find_element(by=By.CLASS_NAME, value="pageImageDisplay")
        checkpoint.click()
        # scrolling ~25 pages
        for i in range(25):
...
def get_cookie(url):
    ...
    """
    cookies = []
    # Instantiate the webdriver and open the URL
    #driver = webdriver.Chrome()  # or Firefox, depending on your setup
    driver.get(url)
    driver.refresh()

    # Iterate over the requests captured by selenium-wire
    for request in driver.requests:
        if request.headers:
    ...
    return cookies[0] if cookies else None  # Return the first cookie found, or None if no cookies found
...
if __name__ == "__main__":
    ...
    #service = Service(executable_path='/home/m0dev/dev/google-books-downloader/chromedriver127')
    service = Service(executable_path='./chromedriver127')
    options = webdriver.ChromeOptions()
    options.add_argument('--headless')
    options.add_argument('--non-sandbox')
